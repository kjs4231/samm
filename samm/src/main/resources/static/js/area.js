...
function paintingFestival(code){
	...
	if(code == null || code ==""){
		...
		for(i=0; i < code.length; i++){	
			text += '<div class="col-md-4 ftco-animate" > <div class="project-wrap"><a class="img"style="background-image:url('+code[i].firstimage+ ')"><span class="price">진행중</span></a><div class="text p-4"><span class="days"><span>' +code[i].eventstartdate+ '</span> ~ <span>'+code[i].eventstartdate+'</span></span><h3><a href="#">'+code[i].title+'<input name="contentid" hidden value="'+code[i].contentid+'"></a></h3><p class="location"><span class="fa fa-map-marker"></span><span>'+code[i].addr1+'</span></p></div></div></div>'		
		}
	}
	return text;
